"""
Phase 3-4B: Corrupted evaluation for Co-TSFA-style TimesNet on ETTh1.

This script loads the Co-TSFA-style pilot checkpoint and evaluates it under
the same clean and corrupted test conditions used in Phase 2.

Run from repository root:

    python custom/evaluation/evaluate_cotsfa_corrupted_etth1.py
...
def find_cotsfa_checkpoint() -> Path:
    """
    Find the Co-TSFA-style pilot checkpoint.
    """
    checkpoint_path = ROOT / "checkpoints_cotsfa" / "cotsfa_etth1_96" / "checkpoint.pth"

    if not checkpoint_path.exists():
        raise FileNotFoundError(
            f"Co-TSFA checkpoint not found: {checkpoint_path}\n"
            "Run custom/training/train_cotsfa_etth1.py first."
        )

    return checkpoint_path


def load_checkpoint_once(model: CoTSFATimesNet, checkpoint_path: Path, device: torch.device) -> None:
    """
...
def main() -> None:
    ...
    print(f"Loaded Co-TSFA checkpoint path: {checkpoint_path}")
    ...
    for condition in conditions:
        ...
        result["training_setting"] = "pilot_2epochs_80batches_lambda0.1"
    ...
    out_path = ROOT / "experiment_logs" / "phase3_cotsfa_corrupted_eval_results.csv"
